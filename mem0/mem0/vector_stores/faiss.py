...
class FAISS(VectorStoreBase):
    ...
    def search(
        ...
        """
        # NOTE: search 会读 index/docstore/index_to_id，必须统一加锁，避免与 insert/update/delete/_save 交叉。
        with self._lock:
            if self.index is None:
                raise ValueError("Collection not initialized. Call create_col first.")
            ...
            if filters:
                fetch_k = min(self.index.ntotal, max(limit * 20, limit))
            else:
                fetch_k = min(self.index.ntotal, limit)

            scores, indices = self.index.search(query_vectors, fetch_k)

            candidates = self._parse_output(scores[0], indices[0], fetch_k)

            if filters:
                filtered = [c for c in candidates if self._apply_filters(c.payload, filters)]
    ...
    def delete(self, vector_id: str):
        ...
        with self._lock:
            ...
            for idx, vid in list(self.index_to_id.items()):
                if vid == vector_id:
                    index_to_delete = idx
                    break
