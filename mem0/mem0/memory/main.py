        ...
        *,
        ...
        with concurrent.futures.ThreadPoolExecutor() as executor:
            ...
            try:
                ...
                logger.warning("Graph add failed; continue without graph. err=%s", e, exc_info=True)
                graph_result = None
        ...
        try:
            for resp in new_memories_with_actions.get("memory", []):
                ...
                try:
                    ...
                    raw_id = resp.get("id")
                    key = str(raw_id).strip() if raw_id is not None else None
                    real_uuid = temp_uuid_mapping.get(key) if key is not None else None

                    if event_type in ("UPDATE", "DELETE") and not real_uuid:
                        logger.warning(
                            f"Skip {event_type}: unknown temp id={raw_id}, available=[0..{len(temp_uuid_mapping)-1}]"
                        )
                        continue
                    ...
                    if event_type == "ADD":
                        ...
                        self._delete_memory(memory_id=real_uuid)
                        returned_memories.append(
                            {
                                "id": real_uuid,
                                "memory": action_text,
            ...
            for resp in new_memories_with_actions.get("memory", []):
                ...
                try:
                    ...
                    if event_type == "ADD":
                        ...
                        memory_tasks.append((task, resp, "UPDATE", real_uuid))
                    elif event_type == "DELETE":
                        task = asyncio.create_task(self._delete_memory(memory_id=real_uuid))
                        memory_tasks.append((task, resp, "DELETE", real_uuid))
                    elif event_type == "NONE":
                        # Even if content doesn't need updating, update session IDs if provided
                        memory_id = real_uuid
                        if memory_id and (metadata.get("agent_id") or metadata.get("run_id")):
