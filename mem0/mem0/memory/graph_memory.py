...
_REL_BAD = re.compile(r"[^0-9A-Za-z_]")
def _sanitize_rel_type(name: str) -> str:
    if not name:
        return "RELATED_TO"
    name = name.strip().replace("`", "")  # 防止破坏 cypher
    name = _REL_BAD.sub("_", name)
...
class MemoryGraph:
    ...
    def _delete_entities(self, to_be_deleted, filters):
        ...
        for item in to_be_deleted:
            ...
            raw_rel = (item.get("relationship") or "").strip()

            # 空关系：不要拼进 Cypher，直接跳过（否则就会出现 -[r:]->）
            if not source or not destination or not raw_rel:
                logger.warning(f"Skip invalid delete spec: {item}")
                continue

            relationship = _sanitize_rel_type(raw_rel)
            ...
            try:
                ...
                logger.warning(f"Delete relation failed; skip. item={item}, err={e}")
                continue
    ...
    def _add_entities(self, to_be_added, filters, entity_type_map):
        ...
        for item in to_be_added:
            ...
            relationship = _sanitize_rel_type(relationship)
    ...
    def _remove_spaces_from_entities(self, entity_list):
        for item in entity_list:
            ...
            raw_rel = (item.get("relationship") or "").lower().replace(" ", "_")
            item["relationship"] = _sanitize_rel_type(raw_rel)
