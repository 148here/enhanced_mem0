...
def main():
    ...
    parser.add_argument("--sample_rate", type=int, default=None, help="Sample rate for questions (e.g., 5 means only process questions where index % 5 == 0)")
    parser.add_argument("--debug", action="store_true", default=False, help="Enable debug output")
    parser.add_argument("--num_candidates", type=int, default=1, help="Number of candidate answers to generate")
    parser.add_argument("--max_expand_rounds", type=int, default=0, help="Maximum number of expansion rounds for top_k")
    parser.add_argument("--expand_step", type=int, default=4, help="Step size for expanding top_k in each round")
    ...
    if args.technique_type == "mem0":
        if args.method == "add":
            memory_manager = MemoryADD(data_path="dataset/locomo10.json", is_graph=args.is_graph, debug=args.debug)
            memory_manager.process_all_conversations(max_conversations=args.max_conversations)
